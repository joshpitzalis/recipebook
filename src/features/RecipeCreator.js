...
export const RecipeCreator = () => {
  ...
  const update = (myIngredients, index, type, value) => {
  ...
  return (
    <Form onSubmit={handleSubmit} errors={error}>
      ...
      <FormField label="Instructions" name="description">
        <TextArea
          value={description}
        ...
        {ingredients &&
          ingredients.map((val, idx) => (
            <Box key={idx} pad={{ vertical: 'small' }} flex>
              <Button
                alignSelf="end"
                onClick={() => removeIngredient(val.ingredient)}
              >
                Remove
              </Button>
              <TextInput
